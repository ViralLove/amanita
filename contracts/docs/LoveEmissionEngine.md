...
- Ограничение "кто может лайкать" проверяется в `LoveDoPostNFT.addSuperlike` по depth-circle модели (конфигурируемые `K` и `L`, общий root/anchor на глубине `L`)
...
### Кто может ставить superlike (depth-circle gating)
- SSOT проверки находится в `LoveDoPostNFT.addSuperlike`.
- Лайк допускается только если у `liker` и `sellerTo`:
- совпадает root invite-дерева;
- совпадает anchor на глубине `L` (`commonAnchorDepthL`);
- выполняется ограничение `|depth(liker) - depth(sellerTo)| <= K` (`likeDepthDistanceK`).
- Параметры `K/L` админ-конфигурируемые, поэтому политика "кто может лайкать" управляется на уровне сообщества без изменений в `LoveEmissionEngine`.
...
### Социальная модель
- 🔗 Эмиссия только по валидным superlike, прошедшим depth-circle gating в LoveDo
- 🛡️ Защита от манипуляций через проверку root/anchor/depth (параметры `K/L`)
