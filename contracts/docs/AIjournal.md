...
## 📋 План реализации Upgradeable архитектуры для экосистемы Amanita

### **Приоритизация контрактов для трансформации:**

**Контракт #1: AmanitaInternational** (КРИТИЧЕСКИЙ ПРИОРИТЕТ - 1 неделя)
- Инфраструктурный контракт для всей системы локализации
- Данные будут расти постоянно (новые языки, продукты, переводы)
- Бизнес-логика будет эволюционировать (модерация, версионирование, governance)
- Без миграции невозможно обновить логику без потери всех CID маппингов

**Контракт #2: SpiralEngine** (ВЫСОКИЙ ПРИОРИТЕТ - 4 недели)
- Ядро спиральной экономики с критическими социальными данными
- Сложная миграция из-за множества интеграций
- Требует тщательного тестирования

**Контракт #3: ProductRegistry** (СРЕДНИЙ ПРИОРИТЕТ - 2 недели)
- Зависит от AmanitaInternational для локализации
- Относительно простая логика, минимум интеграций

---

### **🌐 Контракт #1: AmanitaInternational (Недели 1)**

#### **День 1-2: Архитектурный анализ**
- [ ] **Анализ текущего контракта**
  - ✅ Базовый контракт создан и протестирован (33/33 теста)
  - Критические данные: `simpleFieldCIDs`, `complexFieldCIDs`, массивы полей
  - Бизнес-логика: set/get/remove/batch операции
  
- [ ] **Определение будущих функций**
  - V3: Модерация переводов (голосование за качество)
  - V4: Версионирование CID (история изменений)
  - V5: Репутация переводчиков (качество вкладов)
  - V6: Governance интеграция (валидация через голосование)

#### **День 3-4: Создание AmanitaInternationalV2**
- [ ] **Трансформация в upgradeable**
  - Наследование: `Initializable`, `AccessControlUpgradeable`, `UUPSUpgradeable`
  - Замена `constructor()` на `initialize()`
  - Реализация `_authorizeUpgrade()` с проверкой ADMIN_ROLE
  - Добавление VERSION и UPGRADE_VERSION констант
  
- [ ] **Сохранение storage layout**
  - Все маппинги в том же порядке
  - Добавление gap для будущих версий
  - Валидация отсутствия коллизий

#### **День 5: Деплой и интеграция**
- [x] **Деплой через 3-Contract Proxy Architecture**
  - ✅ Storage контракт (персистентные данные)
  - ✅ LogicV1 контракт (бизнес-логика с immutable storage)
  - ✅ Proxy контракт (реестр адресов + делегирование)
  - ✅ Авторизация Proxy в Storage (PROXY_ROLE)
  - ✅ Регистрация в AmanitaRegistry как "AmanitaInternational"
  
- [x] **Интеграция с deploy_full.js**
  - ✅ Добавлена функция `deployAmanitaInternational()`
  - ✅ Обновлены `SUPPORTED_CONTRACTS` и `CONTRACT_ENV_MAPPING`
  - ✅ Интеграция в action=1 (полный деплой)
  - ✅ Вывод всех 3 адресов в логи для .env
  - ✅ Автоматическая регистрация Proxy в MagicRegistry

#### **День 6-7: Тестирование обновлений**
- [ ] **Создание mock V3**
  - Добавить новую функцию (например, `setTranslationQuality()`)
  - Сохранить все данные из V2
  - Тестовый контракт для проверки upgrade процесса
  
- [ ] **Upgrade тесты**
  - Деплой V2 → установка данных → обновление до V3
  - Проверка сохранности всех маппингов
  - Проверка работы новых функций V3
  - Проверка авторизации обновлений

**Критерии успеха:**
- ⚠️ 83% тестов проходят (34/41 - требуется доработка 7 тестов)
- ✅ Все данные сохраняются после обновления
- ✅ Gas usage оптимален (immutable storage)
- ✅ Proxy зарегистрирован в AmanitaRegistry
- ✅ Интеграция с deploy_full.js завершена
...
# Отдельный деплой только AmanitaInternational
CONTRACT_NAME=AmanitaInternationalProxy npx hardhat run scripts/deploy_full.js 5 --network polygon
```

**После деплоя добавить в .env:**
```bash
AMANITA_INTERNATIONAL_PROXY_ADDRESS=0x...  # Точка входа
AMANITA_INTERNATIONAL_STORAGE_ADDRESS=0x...  # Хранилище
AMANITA_INTERNATIONAL_LOGIC_V1_ADDRESS=0x...  # Логика V1
```

---

### **⚙️ Контракт #2: SpiralEngine (Недели 2-5)**
...
#### **Неделя 3: Реализация Proxy Layer**
...
#### **Неделя 4: Система миграции данных**
...
#### **Неделя 5: Тестирование и валидация**
...
### **🛍️ Контракт #3: ProductRegistry (Недели 6-7)**

#### **Неделя 6: Анализ и трансформация**
- [ ] **Анализ интеграций**
  - Зависимость от SpiralEngine (проверка активации)
  - Зависимость от AmanitaInternational (локализация)
  - Интеграция с IPFS метаданными
  
- [ ] **Создание ProductRegistryV2**
  - Трансформация в upgradeable
...
- [ ] **Тестирование**
  ...
  - Integration с AmanitaInternational
  - Upgrade scenario тесты
  
- [ ] **Деплой и интеграция**
  - Деплой через UUPS Proxy
  - Регистрация в AmanitaRegistry
  - Обновление bot сервисов

---

### **🚀 Production Deployment (Недели 8-9)**

#### **Неделя 8: Подготовка к production**
- [ ] **Security audit**
  - Аудит всех proxy контрактов
...
- [ ] **Деплой AmanitaInternational**
  - Создание Proxy на Polygon mainnet
  - Регистрация в AmanitaRegistry
  - Интеграция с bot сервисами
...
- [ ] **Деплой ProductRegistry**
  ...
  - Финальная проверка экосистемы

---

### **📊 Мониторинг и поддержка (Неделя 10+)**

#### **Система мониторинга**
- [ ] Отслеживание здоровья всех контрактов
- [ ] Мониторинг gas usage
- [ ] Alert система для критических событий
- [ ] Dashboard для визуализации состояния

#### **Документация и обучение**
- [ ] Финальная документация для всех контрактов
- [ ] Обучение команды процедурам обновления
- [ ] Создание runbook для emergency scenarios
- [ ] Видео-туториалы по upgrade процессу
...
### **Контракт #1: AmanitaInternational**
- [x] Базовый контракт AmanitaInternational.sol (33/33 теста, 100% успех)
- [ ] Трансформация в AmanitaInternationalV2 (upgradeable)
- [ ] Деплой через UUPS Proxy
- [ ] Upgrade тесты
- [ ] Документация
...
### **Контракт #2: SpiralEngine**
- [ ] Анализ и подготовка
- [ ] SpiralEngineV2 (upgradeable)
- [ ] Миграция данных
- [ ] Тестирование
- [ ] Деплой
...
### **Контракт #3: ProductRegistry**
- [ ] Анализ интеграций
- [ ] ProductRegistryV2 (upgradeable)
- [ ] Тестирование
- [ ] Деплой

**Прогресс: 0% (0/4 задач)**

### **Общий прогресс экосистемы: 7% (1/14 задач)**
...
2. **Сохранение всех данных** - маппинги CID, социальные связи, репутация
...
**Критический успех**: Вся экосистема AMANITA становится future-proof, с возможностью эволюции без потери данных.

---

**Следующие шаги:**
1. ✅ **AmanitaInternational базовый контракт создан** (33/33 теста)
2. **СЛЕДУЮЩЕЕ:** Трансформация AmanitaInternational в upgradeable (Контракт #1, День 1-2)
3. После завершения #1: Начать SpiralEngine трансформацию (Контракт #2)
4. Параллельно с #2: ProductRegistry трансформация (Контракт #3)
