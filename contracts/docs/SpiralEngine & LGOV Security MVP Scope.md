...
- **Инвайты как логические inviteId‑записи доступа** (без ERC721‑минтинга инвайтов):  
...
- **Anti‑Sybil инварианты InviteGraph (Anti Sybil Policy §4.1–4.4):**  
  ...
  - superlikes и социальные сигналы ограничены depth-circle моделью в LoveDo (`liker` относительно `sellerTo`);  
...
- **Circle‑based interaction limits:** superlikes только при depth-circle проверке в `LoveDoPostNFT` (`_isWithinCommunityDistance(liker, sellerTo)` с параметрами `K/L`);  
