...
> **Примечание о модели $LGOV (состояние на 2026-03-12).**  
> - **Текущее on-chain поведение:** в `LoveEmissionEngine.sol` реализована **одноразовая активация** через флаг `lgovClaimed`: после первого успешного `claimLGOV` дальнейшее накопление `lgovAccrued` продолжает происходить, но повторные `claimLGOV` навсегда ревертятся. Это создаёт “мёртвую зону” governance-эмиссии после первого клейма.  
> - **Целевая модель (Model 2 — reputation-gated continuous governance mining):**  
>   - порог 8+ LoveDo постов остаётся **гейтом зрелости** (доступ к governance-майнингу только после доказанной репутации);  
>   - после прохождения порога участник может **многократно активировать накопленный $LGOV**, и его политический вес растёт вместе с вкладом (новые суперлайки → новый `lgovAccrued` → новые `claimLGOV`);  
>   - эта модель лучше согласуется с философией Amanita: *trust first*, *reputation-gated governance*, *долгая репутационная траектория*, а не разовое посвящение.  
> - **Таск на переход к целевой модели:** см. `contracts/docs/analysis/tasks/task-fix-loveemission-lgov-continuous-governance/task-fix-loveemission-lgov-continuous-governance.md`. До выполнения этого таска документация описывает целевое состояние, а для текущего поведения ориентироваться на реализацию `LoveEmissionEngine.sol` (одноразовый `claimLGOV`).
...
- **Социальная проверка**: только через depth-circle gating в `LoveDoPostNFT` (общий root/anchor на глубине `L` и `|depth diff| <= K`)
