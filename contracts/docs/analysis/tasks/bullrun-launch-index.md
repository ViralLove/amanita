...
| 🔵 | AMN-2.4 | [`subtask-4-tests-checkout-redemption-reputation`](./task-implement-amanita-checkout-escrow-redemption/subtask-4-tests-checkout-redemption-reputation.md) | tests | Implemented (Acceptance approved TODO change status and circle color) | Qualification hardening выполнен поверх existing suite: duplicate order reject, AMN refund one-shot, paired refund events, no refund after `Settled`, reputation non-interaction on cancel/refund; `acceptance-verification-amn-2-4.md`; target suites 38 passing, full `npx hardhat test` — 693 passing. |
...
| 🔵 | AMN-2.7 | [`task-implement-amanita-checkout-cancel-refund`](./task-implement-amanita-checkout-cancel-refund/task-implement-amanita-checkout-cancel-refund.md) | implement | Implemented (Acceptance approved TODO change status and circle color) | **Cancel + on-chain refund** реализован: Love escrow авто-возврат buyer при cancel; AMN-путь восстанавливает `sellerDebt` через `orderDebtRepaid`/`restoreOrderDebtOnCancel`; reentrancy-guards. После DG (AMN-2.8) добавлен buyer mint-back refund по `capturedAmanita` + `BuyerOrderAmnRefunded`; `OrderRefunded` сохранён для rail-агрегации; full `npx hardhat test` — 688 passing. |
| 🔵 | AMN-2.8 | [`subtask-1-buyer-amn-credit-refund-options-and-recommendation`](./task-implement-amanita-checkout-cancel-refund/subtask-1-buyer-amn-credit-refund-options-and-recommendation.md) | analyze | Implemented (Acceptance approved TODO change status and circle color) | Decision gate подтверждён (`1A/2A/3B`): модель `mint-back buyer`, формула `capturedAmanita`, event-модель `BuyerOrderAmnRefunded` + `OrderRefunded`; implementation handoff выполнен и применён в runtime/tests. |
