...
describe("AmanitaCheckout lifecycle and roles", function () {
    ...
    it("creates order with Created status and buyer=sender", async function () {
        ...
        expect(order.buyerDeclaredFullPayment).to.be.false;
    });
    ...
    it("enforces valid transition via declareFullPayment + acceptFullPayment", async function () {
        const orderHash = await createOrderAsBuyer("0x00012b");
        await checkout.connect(buyer).declareFullPayment(orderHash);
        await checkout.connect(seller).acceptFullPayment(orderHash);
        const order = await checkout.getOrder(orderHash);
        expect(order.status).to.equal(2n);
        expect(order.buyerDeclaredFullPayment).to.be.true;
        expect(order.sellerAcceptedFullPayment).to.be.true;
    });
    ...
    it("restricts emergency markOrderPaid to DEFAULT_ADMIN_ROLE (not writer-only)", async function () {
    ...
    it("CHECKOUT_WRITER cannot emergency markOrderPaid; can settle after admin paid", async function () {
        ...
        await expectRevert(checkout.connect(outsider).markOrderPaid(orderHash));
        await checkout.connect(deployer).markOrderPaid(orderHash);
        await checkout.connect(outsider).markOrderSettled(orderHash);
        const order = await checkout.getOrder(orderHash);
        expect(order.status).to.equal(3n);
    });
});
