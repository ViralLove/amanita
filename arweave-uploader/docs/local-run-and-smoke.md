...
- Backend (PUT status, callback): если нет `BACKEND_URL` и `NODE_AUTH_TOKEN` — в лог пишется пропуск (`publish.backend.skip`). Для реального вызова bot задайте оба; секрет должен совпадать с приёмником на боте (`NODE_AUTH_TOKEN` и т.д.) — **[backend-integration.md](./backend-integration.md)**.
