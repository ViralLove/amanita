...
def test_contract_names(blockchain_service):
    """
    Тест получения списка контрактов из реестра MagicRegistry.
    Использует getNames() вместо несуществующего getAllContractNames().
    """
    print("\n=== Тест списка контрактов ===")
    
    try:
        # Получаем все имена контрактов (может содержать дубликаты!)
        all_names = blockchain_service.registry.functions.getNames().call()
        
        # Deduplicate (MagicRegistry appends на каждый set)
        contract_names = list(set(all_names))
        
        print(f"📊 Всего записей в registry: {len(all_names)}")
        print(f"📊 Уникальных контрактов: {len(contract_names)}")
        
        assert len(contract_names) > 0, "Список контрактов пуст"
        
        # Выводим все имена для информации
        print("\n📋 Список зарегистрированных контрактов:")
        ...
        for name in EXPECTED_CONTRACTS:
            if name not in contract_names:
                ...
                print(f"⚠️ Контракт {name} отсутствует в реестре")
        
        if missing_contracts:
            print(f"\n⚠️ Отсутствующие контракты: {', '.join(missing_contracts)}")
            print(f"💡 Возможно контракты не были задеплоены или зарегистрированы под другими именами")
            pytest.skip(f"Контракты не найдены: {', '.join(missing_contracts)}")
        
        print(f"\n✅ Все ожидаемые контракты присутствуют в реестре")
...
def test_contract_addresses(blockchain_service):
    """
    Тест получения адресов контрактов из реестра MagicRegistry.
    Использует get(name) вместо несуществующего getAddress(name).
    """
    print("\n=== Тест адресов контрактов ===")
    
    try:
        for name in EXPECTED_CONTRACTS:
            # Получаем адрес из реестра (correct method: get)
            address = blockchain_service.registry.functions.get(name).call()
            
            # Проверяем что адрес не нулевой
            if address == "0x0000000000000000000000000000000000000000":
                print(f"⚠️ {name}: не зарегистрирован (zero address)")
                pytest.skip(f"Контракт {name} не зарегистрирован в MagicRegistry")
            
            # Проверяем что адрес валидный
            assert Web3.is_address(address), f"Невалидный адрес для {name}: {address}"
            
            # Проверяем что по адресу есть байткод (реальный контракт)
...
def test_invite_metadata_functions(blockchain_service):
    ...
    # userInvites не существует как публичная функция, есть getUserInviteCount и getUserInvites
    metadata_functions = [
        'getUserInviteCount',  # Публичная функция для получения количества инвайтов
        'getCircleSize',       # Размер круга активатора
        'getCircleMembers',    # Члены круга активатора
        'violationCount',      # Счетчик нарушений (public mapping getter)
        'totalInvitesMinted'   # Общее количество заминченных инвайтов
    ]
