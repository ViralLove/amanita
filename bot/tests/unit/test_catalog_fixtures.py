...
def test_fixtures_products_json_structure():
    ...
    fixtures_path = Path(__file__).parent.parent / "fixtures" / "products.json"
    ...
    # 🔧 АДАПТАЦИЯ: Обновлены названия полей под новую структуру (business_id вместо id для валидных)
    required_fields = ["business_id", "title", "cid", "categories", "cover_image_url", "forms", "species", "prices", "organic_components"]
    
    for field in required_fields:
        assert field in first_product, f"Продукт должен иметь поле '{field}'"
    
    # ✅ ИСПРАВЛЕНО: Используем business_id для валидных продуктов
    product_id = first_product.get("id", first_product["business_id"])
    logger.info(f"✅ Первый продукт: {first_product['title']} (ID: {product_id})")
    ...
    # ✅ ИСПРАВЛЕНО: Проверяем уникальность ID валидных продуктов (используем business_id)
    valid_product_ids = [p.get("id", p["business_id"]) for p in valid_products]
    assert len(valid_product_ids) == len(set(valid_product_ids)), "Все ID валидных продуктов должны быть уникальными"
    
    # Проверяем уникальность ID невалидных продуктов (используем id если есть, иначе business_id)
    invalid_product_ids = [p.get("id", p.get("business_id")) for p in invalid_products]
...
def test_catalog_info_structure():
    ...
    # ✅ ИСПРАВЛЕНО: Используем business_id для валидных продуктов (или id если есть)
    catalog_info = {
        "products": valid_products,
        "count": len(valid_products),
        "product_ids": [p.get("id", p["business_id"]) for p in valid_products]
    }
    ...
    # ✅ ИСПРАВЛЕНО: Обновлены expected_ids под актуальные business_id из фикстуры
    ...
    # ✅ ИСПРАВЛЕНО: Проверяем структуру каждого продукта (business_id обязателен для валидных)
    for product in catalog_info["products"]:
        assert "business_id" in product, "Продукт должен иметь поле 'business_id'"
...
def test_product_validation_data():
    ...
    # ✅ ИСПРАВЛЕНО: Используем business_id для валидных продуктов (или id если есть как fallback)
    for product in valid_products:
        product_id = product.get("id") or product.get("business_id")
        assert product_id, "ID (business_id) не должен быть пустым"
    ...
    for product in invalid_products:
        ...
        if product_id == "invalid_empty_fields":
            ...
            # Невалидный формат CID в cover_image_url
            # ✅ ИСПРАВЛЕНО: Проверяем только cover_image_url, так как description_cid больше не используется в структуре
