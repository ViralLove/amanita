...
Примеры:
  ...
  # Просмотр конкретного отчета
  node scripts/utils/view_upload_report.js --report data/state/_upload_reports/_upload_report_polygon_2025-10-09T12-00-00-000Z.json
...
function main() {
  ...
  if (!reportPath) {
    ...
    if (!reportPath) {
      console.error("\n❌ Отчеты не найдены в data/state/_upload_reports/");
      console.error("   Запустите: DEPLOY_ACTION=555 node scripts/deploy_full.js 555");
      process.exit(1);
    }
